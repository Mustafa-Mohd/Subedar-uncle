...
const GALLERY_CATEGORIES = [
  {
    ...
    description: "Serene and comfortable retreats tailored for perfect relaxation.",
    images: [
  ...
  },
  {
    ...
    description: "Elegant and welcoming living spaces that make a lasting impression.",
    images: [
  ...
  },
  {
    ...
    description: "Highly functional, modern kitchens designed for efficiency and style.",
    images: [
  ...
  },
  {
    ...
    description: "Smart storage solutions wrapped in seamless aesthetic finishes.",
    images: [
          ...
          {GALLERY_CATEGORIES.length > 0 ? (
            <div ref={gridRef} className="space-y-24">
              {GALLERY_CATEGORIES.map((category, catIndex) => (
                <div key={catIndex}>
                  <div className="mb-10 text-left border-b border-charcoal/5 dark:border-white/5 pb-4">
                    <h2 className="font-display text-3xl md:text-4xl text-charcoal dark:text-white mb-3">
                      {category.title}
                    </h2>
                    <p className="font-body text-charcoal/60 dark:text-white/60">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {category.images.map((src, imgIndex) => (
                      <motion.div
                        key={imgIndex}
                        className="gallery-item relative aspect-square overflow-hidden rounded-2xl group cursor-pointer bg-white dark:bg-white/5 border border-border shadow-soft hover:shadow-hard transition-all duration-500"
                        whileHover={{ y: -5 }}
